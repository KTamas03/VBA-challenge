...
'Note. last action duplicates the first ticker in the list. I have chosen to paste the unique values from cell "I1".
...
Dim uniquetickers As Double
    ...
    uniquetickers = ws.Cells(ws.Rows.Count, "I").End(xlUp).Row
        ...
        'finds the first ticker starting the search at the top of column A and moving down the rows
        begticker = ws.Range("A:A").Find(What:=ws.Cells(k, 9), After:=ws.Range("A1")).Row
        
        'finds the last ticker starting the search at the bottom of column A and moving up the rows
        endticker = ws.Range("A:A").Find(What:=ws.Cells(k, 9), After:=ws.Range("A1"), SearchDirection:=xlPrevious).Row

        'calculate Yearly Change (Closing Price less Opening Price)
        ...
        'calculate Percent Change (Closing Price less Opening Price then divided by Opening Price)
